...
void EditorWindow::open()
{
    ...
    QString fileName  = QFileDialog::getOpenFileName(
        this, tr("Load File"), dir,
        tr("All files") +
            " (*);;Arduino (*.ino);;Asm (*.asm);;GcBasic (*.gcb)");
...
bool EditorWindow::saveAs()
{
    ...
    if (ext == "")
        ...
        extensions =
            "." + ext + "(*." + ext + ");;" + tr("All files") + " (*.*)";

    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Save Document As"), path, extensions);
    if (fileName.isEmpty())
        return false;
...
bool EditorWindow::saveFile(const QString &fileName)
{
    ...
    m_docWidget->setTabText(m_docWidget->currentIndex(),
...
void EditorWindow::tabContextMenu(const QPoint &eventpoint)
{
    ...
    QAction *setCompilerAction =
        menu->addAction(QIcon(":/copy.png"), tr("Set Compiler Path"));
    connect(setCompilerAction, SIGNAL(triggered()), this, SLOT(setCompiler()));

    QAction *reloadAction =
        menu->addAction(QIcon(":/reload.png"), tr("Reload"));
...
{
    qDebug() << "EditorWindow::tabChanged" << m_docWidget->currentIndex()
             << tab;
}
...
void EditorWindow::createActions()
{
    ...
    saveAsAct = new QAction(QIcon(":/saveas.png"),
                            tr("Save &As...\tCtrl+Shift+S"), this);
...
{
    CodeEditorWidget *actW =
        dynamic_cast<CodeEditorWidget *>(m_docWidget->currentWidget());
    if (actW)
...
void EditorWindow::closeTab(int index)
{
    ...
    CodeEditorWidget *actW =
        dynamic_cast<CodeEditorWidget *>(m_docWidget->widget(index));
    m_docWidget->removeTab(index);
    delete actW;

    int last = m_docWidget->count() - 1;
    if (index > last)
        m_docWidget->setCurrentIndex(last);
    else
        m_docWidget->setCurrentIndex(index);
}
...
void EditorWindow::findReplaceDialog()
{
    ...
    if (text != "")
        findRepDiaWidget->setTextToFind(text);
...
void EditorWindow::readSettings()
{
    ...
    m_docWidget->restoreGeometry(
        settings->value("docWidget/geometry").toByteArray());
